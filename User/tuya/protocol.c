...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_TEMP_CURRENT,get_current_temperature()); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_CURRENT,get_current_humidity()); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TVOC,get_TVOC()); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CH2O,get_CH2O()); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CO2,get_CO2()); //VALUE型数据上报;
	
	
//    mcu_dp_bool_update(DPID_UV_STATE,当前杀菌状态); //BOOL型数据上报;
//    mcu_dp_bool_update(DPID_FRAGRANCE_STATE,当前香薰状态); //BOOL型数据上报;
//    mcu_dp_bool_update(DPID_FRAGRANCE,当前香薰); //BOOL型数据上报;
//    mcu_dp_bool_update(DPID_START,当前启动/停止); //BOOL型数据上报;
...
static unsigned char dp_download_mode_handle(const unsigned char value[], unsigned short length)
{
	...
	//extern void set_work_type(WorkType type);
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_auto_handle(const unsigned char value[], unsigned short length)
{
    ...
    auto1 = mcu_get_dp_download_bool(value,length);
    if(auto1 == 0) {
        //开关关
    } else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_AUTO,auto1);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
