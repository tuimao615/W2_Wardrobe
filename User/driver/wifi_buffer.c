...
WIFI_USART_IRQ_HANDLER
{
  ...
  {   
		...
		{
			countTx++;
			USART_SendData(WIFI_USART,UartBuf_RD(&UartTxbuf)); //环形数据缓存发送
		}
		if(UartBuf_Cnt(&UartTxbuf)==0)  
			USART_ITConfig(WIFI_USART, USART_IT_TXE, DISABLE);//假如缓冲空了，就关闭串口发送中断
  }
  else if(USART_GetITStatus(WIFI_USART, USART_IT_RXNE) == SET)
  {
    USART_ClearITPendingBit(WIFI_USART, USART_IT_RXNE);//清除接收中断标志
        
    //此种环形缓冲数组串口接收方式，适用于解包各种数据，很方便。对数据的要求是:
    //发送方必须要求有数据包头，以便解决串口数据无地址的问题
		countRx++;
    Udatatmp = (unsigned char) USART_ReceiveData(WIFI_USART);          //临时数据赋值
